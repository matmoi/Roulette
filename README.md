...
Since I wanted to learn more about the blockchain technology, especially around Ethereum as it got lot of attractions lately, I started to look for tutorials and found this great one to get familiar with syntax and tools dedicated to Ethereum: https://www.ethereum-france.com/ecrire-une-dapp-pour-ethereum-1-smart-contract/ (french only). Unfortunately the project itself is slightly outdated, for instance compilation fails with solidity 0.4.+, so I decided to fork it in order to fix/extend/improve the original code base.

The project proposes a smart contract for a simplified version of a roulette casino game: a player can bet on a single number,and win 35 times his stake, or whether the number is odd or even, and win twice his stake.
